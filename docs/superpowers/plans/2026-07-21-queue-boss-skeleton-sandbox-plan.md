...
**Intent:** GitHub Actions CI (lint + test + build matrix on macOS/Windows/Linux) plus the org-standard Claude review gate, then update `main`'s baseline protection to add the required checks. (`main` already carries a baseline protection — require PR, conversation resolution, block force-push/deletion, 0 approvals, no required status checks yet; C2 UPDATES it to add the checks below.)
...
- Update `main`'s baseline protection (via `gh api`/ruleset) to ADD required status checks = `claude-review`, `lint`, `test`, `build (ubuntu-latest)`, `build (macos-latest)`, `build (windows-latest)`, `e2e` — never require a check before its workflow exists, or PRs wedge at 'Expected'. Baseline already sets `required_review_thread_resolution: true`, `required_approvals: 0`, squash only.

**TDD order:** Workflow YAML is validated by execution, not unit tests. The C2 PR itself is the test: its matrix must go green on all three OSes and `claude-review` must run and post a verdict on the C2 branch (the workflow is present on the PR head). **Add the required checks to `main`'s baseline protection as the LAST step**, only after the checks are proven on the PR.
